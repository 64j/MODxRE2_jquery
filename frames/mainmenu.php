...
if ($modx->hasPermission('edit_template') || $modx->hasPermission('edit_snippet') || $modx->hasPermission('edit_chunk') || $modx->hasPermission('edit_plugin') || $modx->hasPermission('file_manager')) {
    $sitemenu['elements'] = array('elements', 'main', '<i class="fa fa-th fa-lg"></i>' . $_lang['elements'], '#elements', $_lang['elements'], 'return false;', '', '', 0, 20, '');
}

if ($modx->hasPermission('exec_module')) {
    $sitemenu['modules'] = array('modules', 'main', '<i class="fa fa-cogs fa-lg"></i>' . $_lang['modules'], '#modules', $_lang['modules'], 'return false;', '', '', 0, 30, '');
}
...
if ($modx->hasPermission('edit_user') || $modx->hasPermission('edit_web_user') || $modx->hasPermission('edit_role') || $modx->hasPermission('access_permissions') || $modx->hasPermission('web_access_permissions')) {
    $sitemenu['users'] = array('users', 'main', '<i class="fa fa-users fa-lg"></i>' . $_lang['users'], '#users', $_lang['users'], 'return false;', 'edit_user', '', 0, 40, '');
}
...
if ($modx->hasPermission('bk_manager') || $modx->hasPermission('remove_locks') || $modx->hasPermission('import_static') || $modx->hasPermission('export_static') || $modx->hasPermission('settings')) {
    $sitemenu['tools'] = array('tools', 'main', '<i class="fa fa-wrench fa-lg" aria-hidden="true"></i>' . $_lang['tools'], '#tools', $_lang['tools'], 'return false;', '', '', 0, 50, '');
}
...
if ($modx->hasPermission('remove_locks')) {
    $sitemenu['remove_locks'] = array('remove_locks', 'tools', '<i class="fa fa-hourglass"></i>' . $_lang['remove_locks'], 'javascript:modx.removeLocks();', $_lang['remove_locks'], 'this.blur();', 'remove_locks', '', 0, 20, '');
}
